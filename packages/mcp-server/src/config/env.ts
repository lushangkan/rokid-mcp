...
function readRelayHttpAuthToken(env: NodeJS.ProcessEnv): string {
  const token = readRequiredString(env, "RELAY_HTTP_AUTH_TOKEN").trim();
  if (!token) {
    throw new Error("Environment variable RELAY_HTTP_AUTH_TOKEN must not be blank");
  }
...
export function readMcpEnv(env: NodeJS.ProcessEnv = process.env): McpEnv {
  return {
    ...
    relayHttpAuthToken: readRelayHttpAuthToken(env),
